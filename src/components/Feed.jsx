...
function Feed({ children }) {
  ...
  // console.log(children.profilePicture)
  //   useEffect(() => {
  //     const getImage = async () => {
  //       const result = await axios.get(
  //         "http://localhost:3000/api/images/"+children.profilePicture,
  //         {
  //           withCredentials: true,
  //         }
  //       );
  //       children.profilePicture = result.data
  //     }
  // getImage()
  //   },[]);
  const handleAstro = () => {
    localStorage.setItem("partnerHoroscope", children.horoscope);
    localStorage.setItem("partnerImage", children.profilePicture && null);
    navigate("/astromatch");
  };
  ...
  const handleRequest = async () => {
    try {
      const result = await axios.post(
        "http://localhost:3000/friends/send-request",
      ...
      setTimeout(() => {
        if (result.data.message === "Already request processed")
          setAlertContent("Already sent");
        else setAlertContent("Request Sent");
        setAlertOpen(true);
      }, 200);
    } catch (error) {
      console.log(error);
    ...
    <>
      ...
      <div className="md:grid md:grid-cols-2 lg:grid-cols-3 lg:gap-4 md:gap-2 md:overflow-scroll md:max-h-[82vh]">
        ...
        <div className="md:px-4 md:col-span-2 lg:col-span-1">
          <div className="flex">
            <CarouselDefault src={children.profilePicture} />
          </div>
